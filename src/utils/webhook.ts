...
import type { APIEmbed, APIEmbedField, APIMessage, MessagePayload, WebhookMessageCreateOptions, WebhookMessageEditOptions } from "discord.js";
...
export function generateEmbed(event: Event): APIEmbed {
  ...
  return {
    ...
    image: {
      url: event.imageURL || "https://bths-repair.org/icon.png",
    },
    timestamp: new Date().toISOString(),
    url: `https://bths-repair.org/events/${event.id}`,
    fields
  };

}

export async function sendMessage(options: string | MessagePayload | WebhookMessageCreateOptions): Promise<APIMessage> {
...
export async function editMessage(id: string, options: string | MessagePayload | WebhookMessageEditOptions): Promise<APIMessage> {
